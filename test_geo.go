...
func geoDecodeScore(score uint64) (float64, float64) {
	// Extract latitude bits (they were shifted left by 1 during encoding)
	y := score >> 1
	// Extract longitude bits (they were in the original positions)
	x := score

	// Compact both latitude and longitude back to 32-bit integers
	gridLongitudeNumber := compactInt64ToInt32(x)
	gridLatitudeNumber := compactInt64ToInt32(y)
